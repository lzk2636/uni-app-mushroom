...
export default Vue.extend({
  ...
  methods: {
    async getVcode(){
      ...
      if(resCode.data.status===0){
        uni.showToast({
          title:resCode.data.vcode.toString(),
          icon:"none",
          duration:2000
        })
      }
    ...
    async phoneLogin(){
      ...
      if(vcode.data.status===0){
        uni.showToast({
          title:vcode.data.message,
          icon:"none",
          duration:500
        })
        uni.setStorageSync("my_token",vcode.data.token)
        uni.reLaunch({
